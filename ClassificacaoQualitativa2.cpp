...
int main() {
    ...
    string classif_qualitativa;
    if (classif < 0 || classif > 20) {
        cout << "ATENÇÃO: classificação " << classif << " INVÁLIDA!\n";
        return 2;       // termina programa com erro
    }
    else if (classif <= 4) {
        classif_qualitativa = "Fraco";
    }
    else if (classif <= 9) {
        classif_qualitativa = "Insuficiente";
    }
    else if (classif <= 14) {
        classif_qualitativa = "Suficiente";
    }
    else if (classif <= 20) {
        classif_qualitativa = "Bom";
    }
    cout << "Classificação \"" << classif_qualitativa << "\"\n";
}